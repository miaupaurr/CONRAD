...
public class DefectPixelInterpolation {
	...
	{      
		...
		for(int i = 0; i < maxIter; i++)
		{
			...
			else
			{
				...
				//FHat = N * (G(s1,t1) * W(0,0) - G(s1,t1)* * W(2s1,2t1)) / (|W(0,0)|^2 - |W(2s1,2t1)|^2)
		...
		for(int x = 0; x < result.getWidth(); x++) {
			for (int y = 0; y < result.getHeight(); y++) {
				if (mask.getAtIndex(x, y) == 0) {
					result.setAtIndex(x, y, FHat.getRealAtIndex(x, y));
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/ji77toka/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/testimg.bmp";
		String filenameMask = "/proj/i5dmip/ji77toka/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/mask.bmp";
